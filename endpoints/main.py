...
import pyodbc  # ODBC Module which help to connect with DB
...
def get_db_conn():
    try:
        ...
        # Here we are trying to make same connection string which we have in Azure DB configs as shown in first part
        connection_string = (
            f"Driver={ODBC_DRIVER};"
            f"Server=tcp:{DATABASE_URL},1433;"
            f"Database=endpoint_db;"
            f"Uid=endpoint-admin;"
            f"Pwd=Summer#12;"
            f"AccessToken={token};"
        )
        ...
        logger.error(f"Error while making db connection: {str(e)}")
        raise HTTPException(status_code=401, detail="Could not established DB connection")
